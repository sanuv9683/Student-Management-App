/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.ijse.ijsetrans.repo.impl;

import com.ijse.ijsetrans.core.Admin;
import com.ijse.ijsetrans.core.Exam;
import com.ijse.ijsetrans.repo.ExamRepo;
import java.io.Serializable;
import java.util.List;
import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

/**
 *
 * @author Sanu Vithanage
 */
@Repository
public class ExamRepoImpl implements ExamRepo {

    @Autowired
    private SessionFactory sessionFac;

    private Session getFactory() {
        return sessionFac.getCurrentSession();
    }

    @Override
    public boolean add(Exam dto) throws Exception {
        Serializable save = getFactory().save(dto);
        if (save != null) {
            return true;
        } else {
            return false;
        }
    }

    @Override
    public boolean merge(Exam dto) throws Exception {
        getFactory().merge(dto);
        return true;
    }

    @Override
    public boolean saveUpdate(Exam dto) throws Exception {
        getFactory().saveOrUpdate(dto);
        return true;
    }

    @Override
    public boolean delete(int id) throws Exception {
        getFactory().delete(id);
        return true;
    }

    @Override
    public boolean update(Exam dto) throws Exception {
        getFactory().update(dto);
        return true;
    }

    @Override
    public Exam search(int id) throws Exception {
        Exam admin = (Exam) getFactory().get(Exam.class, id);
        return admin;
    }

    @Override
    public List<Exam> getList() throws Exception {
        List<Exam> list = getFactory().createQuery("From Exam a").list();
        return list;
    }

    @Override
    public List<Exam> advancedSearch(String name) throws Exception {
        List<Exam> list = getFactory().createQuery("From Exam u where u.firstName like '" + name + "%' or u.email like '" + name + "%' or u.lastName like '" + name + "%'").list();
        return list;
    }

    @Override
    public Exam searchForLogin(String email, String pass) throws Exception {
        List<Exam> list = getFactory().createQuery("From Exam a where a.email='" + email + "' and a.pass='" + pass + "'").list();
        Exam temp = null;
        for (Exam admin : list) {
            temp = admin;
        }
        return temp;
    }

    @Override
    public List<Exam> getListA(String query) throws Exception {
        List<Exam> list = getFactory().createQuery(query).list();
        return list;
    }

    @Override
    public void flush() throws Exception {
        getFactory().flush();
    }

}
